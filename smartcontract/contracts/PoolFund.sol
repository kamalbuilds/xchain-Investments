...
contract PoolFund {
    ...
    // Enums
    enum PoolStatus { Active, Closed, Paused }
    enum TransactionType { Deposit, Withdrawal, Bid, Penalty, Distribution }
    ...
    mapping(uint256 => mapping(uint256 => mapping(address => bool))) public hasContributed; // poolId => cycle => member => bool
    mapping(uint256 => mapping(uint256 => mapping(address => Bid))) public poolBids; // poolId => cycle => bidder => Bid
    ...
    event ContributionMade(uint256 poolId, address member, uint256 amount, uint256 cycle);
    event BidSubmitted(uint256 poolId, address bidder, uint256 amount, uint256 cycle);
    event VoteCast(uint256 poolId, address voter, address bidder, uint256 cycle);
    event PayoutDistributed(uint256 poolId, address winner, uint256 amount, uint256 cycle);
   ...
   constructor(IWorldID _worldId, string memory _appId) {
    ...
    function createPool(PoolParameters memory params) public {
        ...
        pool.bidSubmissionDeadline = block.timestamp + params.bidSubmissionDeadline * 1 days;
    ...
    function joinPool(uint256 _poolId) public payable {
        ...
        require(msg.value == pool.commitmentDeposit, "Incorrect commitment deposit");
    ...
    function contribute(uint256 _poolId) public payable {
        ...
        require(!hasContributed[_poolId][cycle][msg.sender], "Already contributed this cycle");
    ...
    function submitBid(uint256 _poolId, uint256 _bidAmount) public {
        ...
        require(member.isActive, "Not a member of the pool");
        require(_bidAmount >= pool.minBidAmount && _bidAmount <= pool.maxBidAmount, "Bid amount out of range");
        require(block.timestamp <= pool.bidSubmissionDeadline, "Bidding period over");
        require(!poolBids[_poolId][cycle][msg.sender].exists, "Bid already submitted");
...
// Voting function with WorldID verification
    function voteOnBid(
        ...
        require(member.isActive, "Not a member of the pool");
        require(hasContributed[_poolId][cycle][msg.sender], "Must contribute before voting");
    ...
    function finalizeCycle(uint256 _poolId) public {
        ...
        require(pool.status == PoolStatus.Active, "Pool is not active");
        uint256 cycle = pool.currentCycle;
        require(block.timestamp > pool.bidSubmissionDeadline, "Bidding period not over");
    ...
    function determineWinner(uint256 _poolId, uint256 cycle, address[] storage bidders) internal view returns (address) {
 ...
 function verifyVoter(
    ...
    function alreadyVoted(Pool storage pool, uint256 cycle, uint256 nullifierhash) internal view returns (bool) {
    ...
    function processPayout(uint256 _poolId, address winnerAddress, uint256 cycle) internal {
        Pool storage pool = pools[_poolId];
    ...
    function resetCycleData(uint256 _poolId) internal {
        ...
        pool.currentCycle += 1;
        pool.bidSubmissionDeadline = block.timestamp + pool.depositPeriodDays * 1 days;
    ...
    function recordTransaction(TransactionInput memory input) internal {
        ...
        Transaction memory txn = Transaction({
            ...
            txHash: keccak256(abi.encodePacked(input.memberAddress, block.timestamp, transactionCounter)),
    ...
    function applyPenalty(uint256 _poolId, address _memberAddress, uint256 _penaltyAmount) internal {
        Pool storage pool = pools[_poolId];
